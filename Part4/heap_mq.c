...
// MODIFIACTION: ADDED COMPILER SWITCH
#define USE_FIFO 1
#ifdef USE_FIFO
  #define SCHEDULE_USED SCHED_FIFO
#else
  #define SCHEDULE_USED SCHED_OTHER
#endif
// END MODIFICATION
...
void main(void)
{
  ...
  // MODIFIED
  rt_max_prio = sched_get_priority_max(SCHEDULE_USED);
  rt_min_prio = sched_get_priority_min(SCHEDULE_USED);
  ...
  rc = pthread_attr_setschedpolicy(&attr_receive, SCHEDULE_USED); // MODIFIED
